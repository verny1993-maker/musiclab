...
def main():
    ...
    enriched = sum(
        1 for t in tracks if t.get("meta", {}).get("meta_status") == "enriched"
    )
    unmatched = sum(
        1 for t in tracks if t.get("meta", {}).get("meta_status") == "unmatched"
    )
    has_discogs = sum(
        1
        for t in tracks
        if t.get("meta", {}).get("label") or t.get("meta", {}).get("discogs_url")
    )
    has_lastfm = sum(
        1 for t in tracks if "lastfm_genres" in t.get("meta", {}).get("sources", {})
    )
    has_mb = sum(
        1
        for t in tracks
        if "musicbrainz_genres" in t.get("meta", {}).get("sources", {})
    )
    ...
    labels = [
        t.get("meta", {}).get("label") for t in tracks if t.get("meta", {}).get("label")
    ]
