...
def search_audio_track(query: str) -> tuple[str | None, str]:
    for src, prefix, timeout in [
        ("soundcloud", "scsearch1:", 20),
        ("youtube", "ytsearch1:", 30),
    ]:
        try:
            cmd = [
                sys.executable,
...
def download_audio(url: str, output_path: Path) -> bool:
    ...
    try:
        cmd = [
            ...
            "yt_dlp",
            "-f",
            "bestaudio[filesize<30M]",  # hard limit, no fallback
            "-o",
            raw_stem + ".%(ext)s",
            url,
            "--no-playlist",
...
def call_analyze(wav_path: Path) -> dict | None:
    ...
    for attempt in range(3):
        try:
            resp = requests.post(
                ANALYZE_URL, json={"filepath": docker_path}, timeout=300
            )
...
def save_error(conn, track_id, artist, title, error, stage):
    conn.execute(
        "INSERT INTO audio_analysis_errors (track_id,artist,title,error,stage) VALUES (?,?,?,?,?)",
...
def analyze_track(track: dict, db_path: Path, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        try:
            ...
            if not audio_url:
                save_error(
                    conn, track_id, artist, title, "No results (SC+YT)", "search"
                )
            ...
            if not downloaded and source_type == "soundcloud":
                logger.debug(
                    "SC download failed, trying YT fallback for %s", query[:40]
                )
                yt_url, yt_source = _search_yt_only(query)
                if yt_url:
                    audio_url, source_type = yt_url, yt_source
                    downloaded = (
                        download_audio(audio_url, temp_wav) and temp_wav.exists()
                    )
            ...
            result["camelot"] = analysis.get("camelot")
...
def main():
    ...
    logger.info(
        "To process: %d (already: %d)", len(to_process), len(tracks) - len(to_process)
    )
    conn.close()

    if args.dry_run:
        for t in to_process[:10]:
            logger.info("  %s — %s", t["artist"], t["name"])
        return
    ...
    if args.workers > 1:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            futures = {
                pool.submit(analyze_track, t, db_path, False): t for t in to_process
            }
            for fut in as_completed(futures):
                ...
                if processed % 40 == 0:
                    ...
                    subprocess.run(
                        ["docker", "restart", "music-analysis-music-analysis-1"],
                        capture_output=True,
                        timeout=30,
                    )
                    time.sleep(10)
        ...
        for i, t in enumerate(to_process):
            ...
            if r["status"] == "ok":
                ...
                err += 1
                logger.warning(
                    "✗ [%d/%d] %s  %s", i + 1, total, t["artist"][:20], r["status"]
                )
    ...
    logger.info("=" * 50)
    logger.info(
        "DONE: %d OK, %d errors, %.0fs (%.0fs/track)",
        ok,
        err,
        elapsed,
        elapsed / max(1, total),
